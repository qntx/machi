...
impl AgentRegistry {
    ...
    pub fn resolve_from_cwd(cwd: impl AsRef<Path>) -> Result<Self, OvoError> {
    ...
    pub fn require(&self, name: &str) -> Result<&AgentDefinition, OvoError> {
    ...
    pub fn discover_dir(self, root: impl AsRef<Path>, strict: bool) -> Result<Self, OvoError> {
        let found = discover_in_dir(root, strict)?;
        Ok(self.merge(&Self::from_definitions(found)))
    }

    /// Discover `{cwd}/.ovo/agents` when present.
    ///
    /// # Errors
    ///
    /// Propagates discovery failures.
    pub fn discover_project(self, cwd: impl AsRef<Path>) -> Result<Self, OvoError> {
    ...
    ///
    /// Discovery I/O failures.
    pub fn resolve_layers(self, cwd: impl AsRef<Path>) -> Result<Self, OvoError> {
