...
///
/// Enforced by [`ovo_runtime::TurnRuntime`] via stop gates: when the model
...
pub enum AgentSource {
    ...
    Builtin,
    /// User home `~/.ovo/agents`.
    User,
    /// Project `.ovo/agents` (cwd → repo root walk).
    #[default]
    Project,
}
...
impl AgentDefinition {
    ...
    /// # Errors
    ///
    /// Returns [`ovo_types::OvoError`] when name/model empty or `max_steps` is zero.
    pub fn validate(&self) -> Result<(), ovo_types::OvoError> {
