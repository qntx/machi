...
//! 1. Builtin catalogue
//! 2. User `~/.ovo/agents` — **skipped** when the name collides with a builtin
//! 3. Project `.ovo/agents` walking cwd → filesystem root (nearer overrides farther)
//!
...
pub const PROJECT_AGENTS_DIR: &str = ".ovo/agents";

/// Relative directory under the user home.
pub const USER_AGENTS_DIR: &str = ".ovo/agents";
...
pub fn parse_definition_markdown(raw: &str) -> Result<AgentDefinition, OvoError> {
...
pub fn load_file(path: impl AsRef<Path>) -> Result<AgentDefinition, OvoError> {
...
pub fn discover_in_dir(
    ...
    strict: bool,
) -> Result<Vec<AgentDefinition>, OvoError> {
    ...
    for entry in rd {
        let entry = entry
            .map_err(|e| OvoError::new(ErrorCode::AgentBuild, format!("read_dir entry: {e}")))?;
        let path = entry.path();
...
/// Discover under `{cwd}/.ovo/agents` when the directory exists.
///
/// # Errors
///
/// Propagates discovery errors; missing directory yields empty list.
pub fn discover_project(cwd: impl AsRef<Path>) -> Result<Vec<AgentDefinition>, OvoError> {
...
/// User home agents directory (`$HOME/.ovo/agents`), when resolvable.
#[must_use]
...
pub fn discover_user() -> Result<Vec<AgentDefinition>, OvoError> {
...
/// Walk `cwd` → filesystem root collecting existing `.ovo/agents` dirs
/// (nearest first).
#[must_use]
...
pub fn resolve_agents(cwd: impl AsRef<Path>) -> Result<Vec<AgentDefinition>, OvoError> {
    let cwd = cwd.as_ref();
...
///
/// Not found or I/O.
pub fn by_name_in_dir(root: impl AsRef<Path>, name: &str) -> Result<AgentDefinition, OvoError> {
    let root = root.as_ref();
...
pub fn by_name_resolved(name: &str, cwd: impl AsRef<Path>) -> Result<AgentDefinition, OvoError> {
    resolve_agents(cwd)?
        .into_iter()
        .find(|d| d.name == name)
        .ok_or_else(|| OvoError::new(ErrorCode::AgentNotFound, format!("agent not found: {name}")))
}
...
fn split_frontmatter(raw: &str) -> Result<(String, String), OvoError> {
...
fn parse_simple_yaml_map(front: &str) -> Result<BTreeMap<String, String>, OvoError> {
